...
public class UserTasklistRepository extends DynamoDbRepository<UserTasklist> {
    ...
    public UserTasklistRepository(@Autowired DynamoDbEnhancedClient dynamoDbEnhancedClient, @Value("${database.tablename}") String tablename) {
        super(dynamoDbEnhancedClient, tablename, UserTasklist.class);
    }

    public List<UserTasklist> findAllByUserId(String userId) {
        return super.findAllByKey(Key.builder().partitionValue(userId).sortValue("tasklist_").build());
    }
}
